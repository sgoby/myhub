...
const (
	...
	// Can be changed.
	DefaultServerVersion = "5.5.10-MyHub"
...
func (l *Listener) handle(conn net.Conn, connectionID uint32, acceptTime time.Time) {
	...
	for {
		...
		case ComStmtExecute:
			...
			err = l.handler.ComQuery(connInterface, query, func(qr *sqltypes.Result) error {
				...
				if !fieldSent {
					...
					//log.Info("Execute fields:",qr.Fields)
					if err := c.writeFields(qr); err != nil {
						return err
					}
				}
				//log.Info("Execute results:",qr.Rows)
		...
		case ComStmtPrepare:
			...
			//stmt := &PrepareStatement{Id:c.statementId}
			c.statementMap[c.statementId] = pStmt
			//
			if err := c.writePrepare(pStmt); err != nil {
